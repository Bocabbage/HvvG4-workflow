...
parser.add_argument('--vg4',
                    help="VG4-ATAC overlapped CSV file.")
parser.add_argument('--ug4',
                    help="UG4-ATAC overlapped CSV file.")
parser.add_argument('--kfroc', default="./KFold_ROC",
                    help="output kfold-val-ROC curve picture file name.")
parser.add_argument('--testroc', default="./test_ROC", type=str,
                    help="output test-ROC curve picture file name.")
parser.add_argument('--testpr', default=None, help="output test-PR \
                    curve picture file name.")
parser.add_argument('--resultdir', default=None,
                    help="output test result-dir.")
parser.add_argument('--seed', default=42, type=int,
                    help="int random-seed.")
parser.add_argument('--kfold', default=5, type=int,
                    help="k of 'KFOLD'.")
parser.add_argument('--test_size', default=0.25, type=float,
                    help="frac of the test size in all dataset.")
parser.add_argument('--mode', default='validation',
                    help="Three mode: validation, train, test")
parser.add_argument('--modeldir', default='./',
                    help="Used with the --mode parameter.")
...
if args.mode == 'validation':
    # Load Data
    train_data, test_data = g4_data_package.get_train_test_set(
        test_size=TEST_SIZE,
        random_state=RANDOM_STATE,
        shuffle=True)
    # train_data = G4_data_package.get_training_set(
    #   trainset_size=8000,
    #   random_state=RANDOM_STATE)
    # test_data = G4_data_package.get_test_set(
    #   testset_size=8000,
    ...
    for i, (clf_name, clf) in enumerate(Classifiers.items(), 1):
        ...
        kfroc_name = args.kfroc + "_{}_randomstate{}.png".format(
            clf_name,
            RANDOM_STATE)
    ...
    Val_Test_Results(
        ...
        res_pic=testroc_name)

elif args.mode == 'train':
    # save_files = ['xgb.joblib', 'svc.joblib', 'rf.joblib', 'lr.joblib']
    save_files = ['svc.joblib']
    train_data, _ = g4_data_package.get_train_test_set(
        test_size="train", random_state=RANDOM_STATE, shuffle=True)
    train_labels = train_data.pop('Label')
    train_data_resample, train_labels_resample = SMOTE().fit_resample(
        train_data.fillna(0.0).to_numpy(),
    ...
    _, test_data = g4_data_package.get_train_test_set(
        test_size="test",
        random_state=RANDOM_STATE,
        shuffle=True)
    ...
    Val_Test_Results(Classifiers, X=None, y=None,
                     X_test=test_data.fillna(0.0).to_numpy(),
                     y_test=test_labels.to_numpy(),
                     res_pic=testroc_name, res_pr=testpr_name,
